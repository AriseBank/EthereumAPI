...
namespace EthereumCore.Azure
{
	...
	public static class AzureStorageUtils
	{
		...
		public static async Task<T> InsertAndGenerateRowKeyAsDateTimeAsync<T>(this INoSQLTableStorage<T> table, T entity, DateTime dateTime, RowKeyDateTimeFormat rowKeyDateTimeFormat = RowKeyDateTimeFormat.Iso) where T : ITableEntity, new()
		{
			var dt = dateTime.ToString(rowKeyDateTimeFormat.ToDateTimeMask());
			var no = 0;
		...
		public static async Task<T> InsertAndCheckRowKeyAsync<T>(this INoSQLTableStorage<T> table, T entity, Func<string> generateRowKey) where T : ITableEntity, new()
		{
			var no = 0;
		...
		public static async Task<T> InsertAndGenerateRowKeyAsTimeAsync<T>(this INoSQLTableStorage<T> table, T entity, DateTime dateTime) where T : ITableEntity, new()
		{
			var dt = dateTime.ToString("HH:mm:ss");
			var no = 0;
		...
		public static class QueryGenerator<T> where T : ITableEntity, new()
		{
			...
			{
				var sqlFilter = $"PartitionKey {QueryComparisons.Equal} '{partitionKey}' and RowKey {QueryComparisons.GreaterThanOrEqual} '{rowKey}'";
		...
		public async static Task<IEnumerable<T>> WhereAsync<T>(this INoSQLTableStorage<T> tableStorage, IEnumerable<string> partitionKeys, DateTime from, DateTime to,
